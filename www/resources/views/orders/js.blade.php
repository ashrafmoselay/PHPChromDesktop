    ...
    $typeaheadSearch.on('typeahead:select', function(e, suggestion) {
        ...
        if (suggestion.is_service == 1) {
            ...
            suggestion.product_unit.map(function(item) {
                ...
                var costv = parseFloat(item.pivot.cost_price);
                if (costv) {
                    var supplier_dicount_percent = parseFloat(suggestion.category
                        .supplier_dicount_percent);
                    if (supplier_dicount_percent) {
                        var priceafterdiscount = costv - (costv * supplier_dicount_percent / 100);
                        costv = parseFloat(priceafterdiscount.toFixed(2));
                    }
                }
                unitsCost[item.id] = costv;
        ...
        templates: {
            ...
            header: function() {
                return '  <div class="s-row s-header">\n' +
                    ...
                    '                    <div class="s-cell {{ !$showcost ? 'hide' : '' }} ">التكلفة</div>\n' +
            ...
            suggestion: function(suggestion) {
                ...
                suggestion.product_unit.map(function(item) {
                    ...
                    if (parseFloat(item.pivot.cost_price)) {
                        ...
                        var supplier_dicount_percent = parseFloat(suggestion.category
                            .supplier_dicount_percent);
                        if (supplier_dicount_percent) {
                            var priceafterdiscount = cost - (cost * supplier_dicount_percent / 100);
                            cost = parseFloat(priceafterdiscount.toFixed(2));
                        }
                    }
                });
                @if ($settings['rounding_up'] == 1)
                ...
                return '  <div class="s-row">\n' +
                    ...
                    '                    <div class="s-cell {{ !$showcost ? 'hide' : '' }}">' +
                    cost + '</div>\n' +
