...
//        return $qty;
    ...
    {
        $result = '';
        $supplier_dicount_percent = $this->category->supplier_dicount_percent;
        foreach ($this->productUnit as $unit) {
            if ((double) $unit->pivot->cost_price) {
                $costPrice = $unit->pivot->cost_price;
                $priceafterdiscount = 0;
                if ($supplier_dicount_percent) {
                    $priceafterdiscount = $costPrice - ($costPrice * $supplier_dicount_percent / 100);
                    $priceafterdiscount = round($priceafterdiscount, 2);
                }
                $result .= $unit->name . ' : ' . round($unit->pivot->cost_price, 2);
                if ($priceafterdiscount) {
                    $result .= ' <b style="color:red;float: left;">' . $priceafterdiscount . '</b>';
                }
                $result .= ' <br/>';
            }
        }
